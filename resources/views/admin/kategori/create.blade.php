...
<div class="container-fluid mt-4">
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-header bg-warning text-dark rounded-top-4 px-4 py-3 d-flex justify-content-between align-items-center">
            <h5 class="mb-0 fw-semibold"><i class="fa fa-plus me-2"></i>Tambah Kategori Baru</h5>
            ...
            <form action="{{ route('admin.kategori.store') }}" method="POST" class="needs-validation" novalidate>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama_kategori" id="nama_kategori"
                        class="form-control rounded-3" value="{{ old('nama_kategori') }}" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="jenis_kategori" id="jenis_kategori" class="form-select rounded-3" required>
                        ...
                        <option value="destinasi" {{ old('jenis_kategori') == 'destinasi' ? 'selected' : '' }}>Destinasi</option>
                        <option value="umkm" {{ old('jenis_kategori') == 'umkm' ? 'selected' : '' }}>UMKM</option>
                        <option value="acara" {{ old('jenis_kategori') == 'acara' ? 'selected' : '' }}>Acara</option>
                    </select>
                </div>
